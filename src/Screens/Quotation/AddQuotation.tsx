...
import { ICustomerInput, leadReourcesOptions } from '../Customers/Customers'
import { IPriceDetails, IProducts } from '../../types/productsTypes'
import { BASE_URI } from '../../App'


const AddQuoantity: FC = () => {
    ...
    const [customerDetails, setCustomerDetails] = useState<ICustomerInput>({
        customerName: "",
        branch: "",
        mobile: "",
        email: "",
    ...
    const calculateTotal = (products: typeof productList, discount: number = 0, additionalCharges: number = 0, paid: number = 0, gst: number = 18) => {
        const totalBeforeAdjustments = products.reduce(
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = await axios.post(`${BASE_URI}/api/quotation/create-quotation`, invoiceData, { withCredentials: true })
            console.log("Quotation submitted successfully", res.data.responses);
